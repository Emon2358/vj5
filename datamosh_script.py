    ...
    try:
        ...
        if match:
            ...
            if not os.path.isabs(actual_downloaded_path):
                actual_downloaded_path = os.path.join(os.getcwd(), actual_downloaded_path)
...
if __name__ == "__main__":
    ...
    if downloaded_path:
        ...
        # set-output は非推奨になったため、環境ファイルを使用する
        # GitHub Actionsの新しい推奨方法: print("variable_name=value" >> $GITHUB_OUTPUT)
        # 今回のケースでは、ファイル名が固定のため、YAMLで直接参照しても問題ありません。
        # もし動的なファイル名をYAMLに渡す必要がある場合は、以下のようにします。
